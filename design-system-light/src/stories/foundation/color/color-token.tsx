...
export const colorTokenTable = (theme: string, tokenLists: string[]) => {
  const preloadedValues: Record<string, string> = {};
  
  try {
    let themeElement: HTMLElement | null = document.querySelector(`.${theme}`);
  ...
  return (
    <div>
      ...
      <div class={`sy-theme ${theme}`}>
        <table part="tb">
          <thead>
            <tr class="header" part="htr">
              <th part="hth hth1">Token</th>
              <th part="hth hth2">Preview</th>
              <th part="hth hth3">Value</th>
            </tr>
          </thead>
          <tbody id="main-area">
            {tokenLists.map((token: string) => {
              const colorValue = preloadedValues[token] || '';
              return (
                <tr part="btr" data-token={token}>
                  <td part="btd">
                    {token}
                    <sy-icon 
                      selectable 
                  ...
                  <td part="btd" class="token-value-cell">{colorValue}</td>
                </tr>
              );
            })}
          </tbody>
