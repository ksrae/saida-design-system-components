...
export const agGridColorTokenTable = (theme: string, tokenLists: any) => {
  const preloadedValues: Record<string, string> = {};
  const agGridTheme = theme === 'sy-theme-dark' ? 'ag-theme-sygrid-dark' : 'ag-theme-sygrid';
  try {
    let themeElement: HTMLElement | null = document.querySelector(`.${agGridTheme}`);
    // 테마 요소가 없으면 임시 생성
    let tempCreated = false;
    if (!themeElement) {
      themeElement = document.createElement('div');
      themeElement.className = agGridTheme;
    ...
    Object.entries(tokenLists).forEach(([category, tokens]) => {
          ...
          <tbody id="main-area">
            {Object.entries(tokenLists || {}).map(([category, tokens]) => {
              ...
              return tokenArray.map((token: string, index: number) => {
                const colorValue = preloadedValues[token] || '';
                return h('tr', { part: 'btr' }, [
                  index === 0 ? h('td', { part: 'btd', rowspan: tokenArray.length }, category) : null,
                  h('td', { part: 'btd' }, 
                    h('div', {}, [
                      h('span', {}, token),
                      h('sy-icon', { 
                        ...
                        h('sy-tooltip', { arrow: true, content: 'copy to clipboard' })
                      ])
                    ])
                  ),
                  h('td', { part: 'btd' }, 
