import { LitElement, CSSResultGroup, css, unsafeCSS, html, nothing } from 'lit';
import { customElement, property } from 'lit/decorators.js';
import '../icon/icon.element';
import globalCSS from './styles/empty.scss?inline';

@customElement('sy-empty')
export class EmptyElement extends LitElement {
  static styles: CSSResultGroup = css`${unsafeCSS(globalCSS)};`
  @property({ type: String }) description!: string; 
  // @property({ type: Boolean, reflect: true }) nodescription: boolean = false;
   
  
  render(){
    return html`
    <div class="empty-wrapper">
    <div class="empty">
      <!-- <span class="icon"><i class="fa-regular fa-sharp fa-inbox"></i></span> -->
    <sy-icon size="xxxlarge"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path d="M139.3 158C140.3 150 147.1 144 155.2 144L484.9 144C492.9 144 499.7 150 500.8 158L525.8 352L440.6 352C427.2 352 414.7 358.7 407.3 369.8L387.2 400L252.9 400L232.8 369.8C225.4 358.7 212.9 352 199.5 352L114.3 352L139.3 158zM112 400L195.2 400L215.3 430.2C222.7 441.3 235.2 448 248.6 448L391.5 448C404.9 448 417.4 441.3 424.8 430.2L444.9 400L528.1 400L528.1 480C528.1 488.8 520.9 496 512.1 496L128 496C119.2 496 112 488.8 112 480L112 400zM155.2 96C123 96 95.8 119.9 91.7 151.8L64.2 364.9L64 366.5L64 480C64 515.3 92.7 544 128 544L512 544C547.3 544 576 515.3 576 480L576 366.5L575.8 365L548.3 151.9C544.2 119.9 517 96 484.8 96L155.2 96z"/></svg></sy-icon>
    ${this.description ? html`<span class="description">${this.description}</span>`: nothing}
    </div>
  </div>
  `;
  }
}