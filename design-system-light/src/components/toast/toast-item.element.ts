...
import globalCSS from "./styles/toast-item.scss?inline";
import "../icon/icon.element";

@customElement('sy-toast-message-item')
export class ToastItemElement extends LitElement {
  static styles: CSSResultGroup = css`
    ${unsafeCSS(globalCSS)};
  `;    
  @property({ type: Boolean, reflect: true}) open = false;
  @property({ type: String, reflect: true}) position: 'topLeft' | 'topRight' | 'bottomLeft' | 'bottomRight' = 'bottomRight';
  @property({ type: String, reflect: true}) variant: 'neutral' | 'success' | 'error' | 'info' | 'warning' = 'neutral';

  @state() closable = false;
  @state() duration = 4500; // 밀리초 단위
  @state() latestTop = false;
  @state() private mouseLeaveDuration = 1500; // 밀리초 단위
  @state() private iconName = '';
  @state() private hasContent: boolean = false;

  @query('slot[name="icon"]') iconSlot !: any;
  ...
  render() {
    ...
    <div class="toast-icon">    
      ${this.variant === 'neutral' ? html`
        <slot name="icon" @slotchange="${this.handleSlotChange}"></slot>
      ` : nothing}
      ${this.variant !== 'neutral' ? html`
        <sy-icon size="xxlarge" class="toast-icon">
          ${unsafeHTML(this.iconName)}
        </sy-icon>`
        : nothing
      }    
    </div>
    <div class="toast-messsage">
      <header>
...
//register close animation 2 seconds before the toast closes
  private registerCloseAnimation() {
    if (this.duration > 2000) {
      this.animationTimer = window.setTimeout(() => {
        this.classList.add('exit-active');
      }, this.duration - 2000);
    }
  }

  private startMouseLeaveTimer() {
    this.clearTimer();
    if (this.mouseLeaveDuration > 0) {
      this.timer = window.setTimeout(() => {
        this.close();
      }, this.mouseLeaveDuration);

      this.animationTimer = window.setTimeout(() => {
        this.classList.add('exit-active');
      }, this.mouseLeaveDuration - 2000);
    }
  }
  ...
  private onMouseEnter() {
    this.clearTimer(); // 마우스가 올라오면 타이머를 클리어하여 닫히지 않도록 함
  }

  private onMouseLeave() {
    this.startMouseLeaveTimer();  // 마우스가 나가면 mouseLeaveDuration 타이머 시작
  }
  ...
  private adjustPosition() {
    const toasts = (Array.from(document.querySelectorAll('sy-toast-message-item')) as ToastItemElement[])
     .filter((toastItem: ToastItemElement) => toastItem.open && toastItem.position === this.position);
    ...
    let totalHeight = 0;
    const maxHeight = window.innerHeight - 32; // Padding 고려
    const toRemove: HTMLElement[] = [];

    toasts.forEach(toastItem => {
      const toastItemElement = toastItem as HTMLElement;
      const toastHeight = toastItemElement.offsetHeight + 16;

      if (totalHeight + toastHeight > maxHeight) {
        toRemove.push(toastItemElement);
      } else {
        totalHeight += toastHeight;
        toastItemElement.style.transform = `translateY(${offset}px)`;
