...
import globalCSS from './styles/date-calendar.scss?inline';

@customElement('sy-date-calendar')
export class DateCalendarElement extends LitElement {
	...
	@property({ type: Object }) datetime: {year: number, month: number, day: number, hour: number, minute: number, second: number} = {
	...
	@property({ type: String }) range?: 'start' | 'end';
	@property({ type: String }) active!: string; 
	@property({ type: Object, reflect: true }) rangestart: {year: number, month: number, day: number } | undefined = undefined;
	@property({ type: Object, reflect: true }) rangeend: {year: number, month: number, day: number } | undefined = undefined;
	@property({ type: String }) hoverDate!: string;
	@property({ type: String }) dateNames: string = 'Su,Mo,Tu,We,Th,Fr,Sa';
	...
	updated(changedProperties: Map<string | number | symbol, unknown>) {
		...
		if (!this.range && changedProperties.has('datetime')) {
		...
		if (!this.range && changedProperties.has('originalDatetime')) {
		...
		if (changedProperties.has('hoverDate')) {
			this.setHoverStyle();
		}
	}
		...
		<div class="date-calendar-container">
			<div class="calendar-header">
				<div class="calendar-header-privious">
					${this.mode === 'day' ? html`<sy-icon size="medium" @click="${this.prevMonth}">
				...
				<div class="calendar-header-next">
					${this.mode === 'day' ? html`<sy-icon size="medium" @click="${this.nextMonth}">
					...
					<sy-icon size="medium" @click="${this.setPost}">
			...
			${
				...
				html `
					...
					<sy-button size="small" variant="borderless" @click=${this.handleToday}>Now</sy-button>
				` : nothing
			}			
			</div>
			</div>
  ...
  private generateCalendarDays() {
	...
	private generateDecades() {
		...
		for (let i = currentCenturyStart - 10; i <= currentCenturyStart + 100; i += 10) {
			...
			const decade = html`
				<li ?selected=${isSelected} ?today=${isToday} class="${i === currentCenturyStart - 10 || i === currentCenturyStart + 100 ? 'not-current-month' : ''}" @click="${
					() => {
		...
		for (let i = 0; i < decades.length; i += 3) {
			rows.push(html`<ul class="calendar-grid">${decades.slice(i, i + 3)}</ul>`);
		}
		return rows;
	}
	...
	private generateYears() {
		...
		for (let i = startDecade - 1; i <= startDecade + 10; i++) {
			...
			const year = html`
				<li ?selected=${isSelected} ?today=${isToday} class="${i === startDecade - 1 || i === startDecade + 10 ? 'not-current-month' : ''}" @click="${
					() => {
						if(i === startDecade - 1) {
							this.prevYear();
		...
		for (let i = 0; i < years.length; i += 3) {
			rows.push(html`<ul class="calendar-grid">${years.slice(i, i + 3)}</ul>`);
		}
		return rows;
	}
	...
	private generateMonths() {
		...
		for (let i = 0; i < 12; i++) {
			...
			const month = html`
				<li ?selected=${isSelected} ?today=${isToday} @click="${() => this.selectMonth(i)}"><span class="date-item">${new Date(0, i).toLocaleString('default', { month: 'short' })}</span></li>`;
				months.push(month);
		}
		const rows = [];
		for (let i = 0; i < months.length; i += 3) {
			rows.push(html`<ul class="calendar-grid">${months.slice(i, i + 3)}</ul>`);
		}
		return rows;
	}
	
	private renderDayNames() {
		if (this.mode !== 'day') return nothing;
		...
		return html`${dayNames.map(name => html`<li class="day-header">${name}</li>`)}`;
	}
	...
	private generateDays() {
		...
		for (let i = 0; i < startDay; i++) {
			...
			days.push(html`<li class="not-current-month" @click="${
				() => {
					this.prevMonth();
					this.selectDate(preYear, preMonth, preDate, false);
				}
			}"><span class="date-item">${preDate}</span></li>`);
		}
		...
		for (let day = 1; day <= numDays; day++) {
			...
			<li 
				...
				?today=${isToday}
				?selected=${isSelected} 
				?inrange=${inRangeForThisDay}
				?rangestart=${isRangeStartDay} 
				?rangeend=${isRangeEndDay} 
				data-date=${dataDate}
				@mouseenter="${() => this.setHoverEvent(dataDate)}"
				@mouseleave="${this.handleMouseLeave}"
				@click="${() => this.selectDate(year, month, day)}">
		...
		while (days.length < totalDays) {
			...
			}"><span class="date-item">${nextMonthDay}</span></li>`);
		...
		for (let i = 0; i < days.length; i += daysPerWeek) {
			weeks.push(html`<ul class="calendar-grid">${days.slice(i, i + daysPerWeek)}</ul>`);
		}
  ...
  private get firstDayOfMonth() {
	...
	private setHeaderText() {
		if (this.mode === 'decade') {
			return html`<span>${Math.floor(this.currentDate?.getFullYear() / 100) * 100}-${Math.floor(this.currentDate?.getFullYear() / 100) * 100 + 99}</span>`;
		} else if (this.mode === 'year') {
			return html`<span @click=${this.changeToDecadeView}>${Math.floor(this.currentDate?.getFullYear() / 10) * 10}-${Math.floor(this.currentDate?.getFullYear() / 10) * 10 + 9}</span>`;
		} else if (this.mode === 'month') {
			return html`<span @click=${this.changeToYearView}>${this.currentDate?.getFullYear()}</span>`;
		} else { // this.mode === 'day'
			return html`	
				<span @click=${this.changeToMonthView}>${this.currentDate?.toLocaleString('default', {year: 'numeric'})}</span>
				<span @click=${this.changeToMonthView}>${this.currentDate?.toLocaleString('default', { month: 'long' })}</span>`;
		}
	}
  ...
  private nextYear() {
	...
	private setHoverStyle() {
			...
			const items = this.shadowRoot?.querySelectorAll('.current-month');
	...
	private handleMouseLeave() {
		const items = this.shadowRoot?.querySelectorAll('.highlight, .highlight-start, .highlight-end');
		items?.forEach(item => {
			item.classList.remove('highlight', 'highlight-start', 'highlight-end');
		});
		this.hoverDate = '';
