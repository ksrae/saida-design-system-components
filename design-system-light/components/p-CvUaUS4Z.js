...
const syPopoverCss = "@charset \"UTF-8\";.sc-sy-popover-h{display:none;position:absolute;background-color:var(--popover-background-enabled);color:var(--popover-text-enabled);box-shadow:var(--box-shadow);padding:var(--spacing-xsmall);z-index:var(--z-index-popover);border-radius:var(--border-radius-medium)}.sc-sy-popover-h .arrow.sc-sy-popover{box-shadow:var(--box-shadow)}sy-popover.sc-sy-popover-h{z-index:var(--z-index-popover);background-color:var(--popover-background-enabled)}sy-popover.arrow.sc-sy-popover-h{z-index:var(--z-index-popover);box-shadow:var(--box-shadow);background-color:var(--popover-background-enabled)}[open].sc-sy-popover-h{display:block}";

const SyPopover = /*@__PURE__*/ proxyCustomElement(class SyPopover extends H {
    ...
    // 상태 관련 필드
    addedToBody = false; // body에 추가되었는지 여부
    isMouseOverParent = false; // 부모 요소 위에 마우스가 있는지
    isMouseOverPopover = false; // 팝오버 위에 마우스가 있는지
    ARROW_HEIGHT = 6; // 화살표 높이 (px)
    closeTimer; // 닫기 타이머 참조
    openTimer; // 열기 타이머 참조
    parentDom; // 부모 요소 참조
    DefaultOpendelay = 0; // 기본 열기 딜레이(ms)
    DefaultClosedelay = 100; // 기본 닫기 딜레이(ms)
    optionClickTimer = null; // 옵션 클릭 타이머 참조
    lastOptionClickTime = 0; // 마지막 옵션 클릭 시간
    lastParentClickTime = 0; // 마지막 부모 클릭 시간
    optionInteractionActive = false; // 옵션 상호작용 활성화 여부
    preventPositionUpdate = false; // 위치 업데이트 방지 플래그
    optionDetectionListenerAdded = false;
    // 디바운스 및 쓰로틀링 시간 상수
    CLICK_DEBOUNCE_TIME = 300; // 옵션 클릭 후 상태 유지 시간(ms)
    OBSERVER_CLEANUP_TIME = 3000; // 옵저버 정리 시간(ms)
    MOUSELEAVE_DELAY = 200; // 마우스 이탈 후 상태 체크 지연 시간(ms)
    // 속성
    arrow = false; // 화살표 표시 여부
    open = false; // 열림 상태 여부
    position = 'bottom'; // 위치
    trigger = 'hover'; // 트리거 방식
    opendelay = 0; // 열기 지연 시간
    closedelay = 100; // 닫기 지연 시간
    // =================================================================
    // 1. sticky 프로퍼티 추가 (기본값 false)
    // =================================================================
    sticky = false;
    arrowElement; // 화살표 요소 참조
    /**
     * 생성자: 이벤트 핸들러 바인딩
     */
    componentWillLoad() {
     ...
     */
    watchProps(_newValue, _oldValue, propName) {
        if (propName === 'trigger') {
            if (!this.open) {
                this.addEvent();
            }
     ...
     */
    render() {
        return h("slot", { key: 'd2877be61ecd75e5c6c02057bfa1fb2f745b5190' });
    }
    /**
     * 팝업을 열기 위한 공개 메서드
     */
    ...
    updatePopoverPosition() {
        if (this.addedToBody && this.parentDom !== document.body && this.parentDom) {
            ...
            requestAnimationFrame(() => {
                ...
                const { position: bestPosition, coords } = this.findBestPosition(positions, this.position, parentRect, popoverRect);
                ...
                if (this.arrow) {
                    this.arrowElement = this.createArrow(bestPosition, parentRect, adjusted ? { top: parseFloat(this.host.style.top), left: parseFloat(this.host.style.left) } : coords, popoverRect, adjusted);
    ...
    findBestPosition(positions, preferredPosition, _parentRect, popoverRect) {
        ...
        const checkDirection = (pos, crds) => {
            ...
            if (pos.startsWith('bottom'))
                return crds.top + popoverRect.height > scrollY + viewportHeight;
            if (pos.startsWith('left'))
                return crds.left < scrollX;
            if (pos.startsWith('right'))
                return crds.left + popoverRect.width > scrollX + viewportWidth;
            return false;
        };
    ...
    adjustForScreenBounds(popoverRect) {
        const viewportWidth = window.innerWidth;
        const viewportHeight = window.innerHeight;
        let adjusted = false;
    ...
    createArrow(position, parentRect, popoverPos, popoverRect, positionAdjusted) {
    ...
    positionArrowRelativeToParent(arrowElement, position, parentRect, popoverPos, popoverRect) {
        let targetX, targetY;
        switch (position) {
            case 'top':
    ...
    calculateAllPositions(parentRect, popoverRect) {
    ...
    setupOptionDetection() {
        const setupContainerListeners = (container) => {
            if (container.__popoverListenersAdded)
                return;
            container.__popoverListenersAdded = true;
            ...
            container.addEventListener('mouseleave', (e) => {
                ...
                if (relatedTarget === this.host || this.host.contains(relatedTarget) || this.isOptionRelated(relatedTarget) || this.parentDom?.contains(relatedTarget))
                    return;
                if (this.trigger === 'hover') {
                    this.optionInteractionActive = false;
                    this.isMouseOverPopover = false;
            ...
            mutations.forEach(mutation => {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        const element = node;
                        const isContainer = element.classList && (element.classList.contains('sy-select-options-container') ||
    ...
    updatePopoverPositionWithDelay() {
        requestAnimationFrame(() => {
    ...
    static get watchers() { return {
        "trigger": ["watchProps"],
        "open": ["watchProps"],
        "opendelay": ["watchProps"],
        "closedelay": ["watchProps"]
    }; }
    static get style() { return syPopoverCss; }
}, [262, "sy-popover", {
        "arrow": [516],
        "open": [516],
        "position": [1],
...
function defineCustomElement() {
    ...
    const components = ["sy-popover"];
    components.forEach(tagName => { switch (tagName) {
        case "sy-popover":
            if (!customElements.get(tagName)) {
                customElements.define(tagName, SyPopover);
            }
            break;
    } });
}

export { SyPopover as S, defineCustomElement as d };
