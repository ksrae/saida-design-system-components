...
function fnGetAssignedNodesContent(host) {
...
function fnGetChildrenByTagName(parentElement, childTagName) {
    if (!parentElement) {
        return [];
    }
    return Array.from(parentElement.children).filter((child) => child.tagName.toLowerCase() === childTagName.toLowerCase());
}
// 특정 태그 이름을 가진 부모 요소가 존재하는지 확인하는 함수
function fnFindClosestParentByTagName(host, tagName) {
...
function fnHasSlotContentByName(host, slotName) {
    if (!host.isConnected)
        return false;
 ...
 */
function fnAssignPropFromAlias(host, ...aliases) {
    try {
...
function fnResolvePropAlias(host, ...aliasAttr) {
    try {
        if (!host)
            return null;
...
export { fnHasSlotContentByName as a, fnFindClosestParentByTagName as b, fnGetAssignedNodesContent as c, fnGetChildrenByTagName as d, fnAssignPropFromAlias as f };
